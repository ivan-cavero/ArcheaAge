...
local live = lbl("ipLive", "", 106, {
  0.6,
  0.78,
  0.6
})
lbl("ipBuild", CFG.build, 130, {
  0.55,
  0.65,
  0.8
})
...
pcall(function()
  win:SetHandler("OnUpdate", function()
    ...
    local fps
    pcall(function()
      fps = UIParent:GetFrameRate()
    end)
    local ok, hms = pcall(function()
    ...
    if tries <= 15 then
      ...
      if okc and type(n) == "number" then
        pcall(function()
          chars:SetText(("Personajes: %d"):format(n))
        end)
        tries = 999
      end
    end
...
local function tryApply(o)
  ...
  pcall(function()
    ...
    if o.color then
      w.style:SetColor(rgbArr(o.color))
    end
  end)
...
if 0 < #pending then
  ...
  pcall(function()
    win:SetHandler("OnUpdate", function()
      ...
      OL("reintento " .. otries .. " \195\130\194\183 quedan=" .. left)
      if left == 0 or 20 <= otries then
        OL("overrides fin (quedan=" .. left .. ", intentos=" .. otries .. ")")
        pcall(function()
          win:ReleaseHandler("OnUpdate")
        end)
        if OVLOG then
          OVLOG:close()
          OVLOG = nil
...
for _, c in ipairs(CUSTOM) do
  pcall(function()
    ...
    if c.type ~= "window" and c.text then
      ...
      if c.color then
        l.style:SetColor(rgbArr(c.color))
      end
