...
local function isWidget(t)
  ...
  if okm and type(mt) == "table" then
    ...
    local oka, ac = pcall(function()
      return mt.AddChild
    end)
    if oka and type(ac) == "function" then
      return true
    end
  end
  return false
...
local function emit(path, o, depth)
  ...
  local jsep = 1 < COUNT and "," or ""
  local escs = tostring(txt or ""):gsub("[%c\"\\]", "")
  W(string.format("%s{\"path\":\"%s\",\"nativeId\":%s,\"w\":%s,\"h\":%s,\"visible\":%s,\"text\":\"%s\"}\n", jsep, path, type(wid) == "number" and wid or "null", type(wd) == "number" and wd or "null", type(ht) == "number" and ht or "null", vis == false and "false" or "true", tostring(escs):sub(1, 60)))
end

local function walk(o, path, depth)
  if 9 < depth or 2500 < COUNT then
    return
  end
  if SEEN[o] then
    return
