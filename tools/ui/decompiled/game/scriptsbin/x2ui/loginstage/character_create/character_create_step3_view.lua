...
local function CreateAbilitySet(parent, index)
  ...
  button:AddAnchor("TOPLEFT", parent, index * characterCreateLocale.abilityButton.anchorSize + characterCreateLocale.abilityButton.offset, 15)
...
function CreateAbilityWindow(id, parent)
  ...
  desc:SetExtent(0, 40)
  desc:AddAnchor("TOP", widget, 0, 130)
  desc:AddAnchor("LEFT", widget, 10, 0)
  desc:AddAnchor("RIGHT", widget, -10, 0)
  ...
  widget.desc = desc
  local abilityTilteTexture = widget:CreateImageDrawable(characterCreateLocale.abilityDesc.path, "artwork")
  abilityTilteTexture:AddAnchor("TOPLEFT", widget, 5, 160)
  widget.abilityTilteTexture = abilityTilteTexture
  local bg = widget:CreateImageDrawable(LOGIN_STAGE_TEXTURE_PATH.REMNANTS, "background")
  ...
  desc.bg = bg
  local abilityTitle = widget:CreateChildWidget("label", "abilityTitle", 0, true)
  ...
  local abilityDesc = widget:CreateChildWidget("textbox", "abilityDesc", 0, true)
  ...
  for i = 1, #ABILITY_TYPE do
    ...
    function button:OnClick()
      ...
      SelectAbility(i)
      ShowAbilitySkillButtons(widget, widget.abilityDesc, i)
    end
...
abilityWnd:AddAnchor("TOPRIGHT", step3.content.rightPanel, -60 + characterCreateLocale.abilityDiagram.bg.anchor[1], 330 + characterCreateLocale.abilityDiagram.bg.anchor[2])
...
diagram:Show(true)
diagram:AddAnchor("CENTER", abilityWnd, 0, 5)
...
local startButton = rightPanel:CreateChildWidget("button", "startButton", 0, true)
startButton:Raise()
startButton:Show(true)
startButton:Enable(false)
ApplyButtonSkinTable(startButton, BUTTON_LOGINSTAGE.GAME_START)
startButton:AddAnchor("BOTTOMRIGHT", rightPanel, 0, 1)

function rightPanel.startButton:OnClick(arg)
  if arg == "LeftButton" then
    ShowCreateConfirmPopup(true)
  end
end

rightPanel.startButton:SetHandler("OnClick", rightPanel.startButton.OnClick)
...
function step3.content:OnVisibleChanged(visible)
  if visible == true then
    ...
    rightPanel.abilityDesc:SetText("")
    rightPanel.abilityTitle:Show(false)
    rightPanel.abilityTilteTexture:SetVisible(false)
    ...
    EnableEditBox(true)
    self.rightPanel.startButton:Show(false)
    if X2LoginCharacter:IsRecustomizing() then
