if X2Util:GetGameProvider() == TRION and X2Locale:GetLocale() == "fr" then
  NEXT_ARROW_TEXTURE_OFFSET.X = -13
  ...
  characterCreateLocale.abilityButton = {
    anchorSize = 110,
    offset = -170,
    fontSize = FONT_SIZE.MIDDLE
  }
  
  function characterCreateLocale:CreateAbilityButton(parent, index, text)
    local colorTable = {
      GetLoginStageAbilityFightButtonFontColor().normal,
      GetLoginStageAbilityMagicButtonFontColor().normal,
      GetLoginStageAbilityWildButtonFontColor().normal,
      GetLoginStageAbilityLoveButtonFontColor().normal,
      GetLoginStageAbilityDeathButtonFontColor().normal,
      GetLoginStageAbilityVocationButtonFontColor().normal
    }
    local button = parent:CreateChildWidget("button", "abilityBtn", index, true)
    local textbox = button:CreateChildWidget("textbox", "textbox", 0, false)
    textbox:SetWidth(80)
    textbox:SetText(text)
    textbox:SetHeight(textbox:GetTextHeight())
    textbox:AddAnchor("BOTTOM", button, "TOP", 0, 23)
    textbox.style:SetFontSize(FONT_SIZE.LARGE)
    local color = colorTable[index]
    textbox.style:SetColor(color.r, color.g, color.b, 1)
    return button
  end
  
  characterCreateLocale.abilityDesc = {
    path = LOGIN_STAGE_TEXTURE_PATH.IMG_TEXT2,
    abilityTitle = {
      coords = {
        UIParent:GetTextureData(LOGIN_STAGE_TEXTURE_PATH.IMG_TEXT2, "Battlerage").coords,
        UIParent:GetTextureData(LOGIN_STAGE_TEXTURE_PATH.IMG_TEXT2, "Sorcery").coords,
        UIParent:GetTextureData(LOGIN_STAGE_TEXTURE_PATH.IMG_TEXT2, "Archery").coords,
        UIParent:GetTextureData(LOGIN_STAGE_TEXTURE_PATH.IMG_TEXT2, "Vitalism").coords,
        UIParent:GetTextureData(LOGIN_STAGE_TEXTURE_PATH.IMG_TEXT2, "Occultism").coords,
        UIParent:GetTextureData(LOGIN_STAGE_TEXTURE_PATH.IMG_TEXT2, "Shadowplay").coords
      }
    }
  }
  characterCreateLocale.abilityDiagram = {
    path = LOGIN_STAGE_TEXTURE_PATH.IMG_TEXT2,
    bg = {
      coords = UIParent:GetTextureData(LOGIN_STAGE_TEXTURE_PATH.IMG_TEXT2, "Diagram").coords,
      anchor = {-7, -7}
    },
    labelExtent = {
      {100, 20},
  ...
  characterCreateLocale.characterName = {
