...
function PlayCharacterVoice(raceIndex, genderIndex)
  StopSound(voiceIndex)
  voiceIndex = PlayUISound(RACE_GENDER_VOICE_TABLE[raceIndex][genderIndex])
...
local function SelectRace(index)
  ...
  if index == RACE_WARBORN then
    step1.content.rightPanel.genderBtn[GENDER_FEMALE]:Enable(false)
  else
  ...
  for k = 1, 2 do
    ...
    if k == index then
      ...
      for i = 1, #RACE_TYPE do
        ...
        if index == GENDER_FEMALE and race == RACE_WARBORN then
        else
...
function SetCustomStyleToUnit(typeIndex, itemIndex, r, g, b, weight)
  local func = {
...
function SetCustomItemToParam(part, index, fixed)
  if part ~= nil then
    if index == nil then
      index = 0
    end
    customizingUnit:SetCustomizingParam(part - 1, index - 1, fixed)
  end
end
...
function InitializeCurrentParam()
  if customizingUnit ~= nil then
    return customizingUnit:InitCustomizerControl()
  end
end
...
local function ShowAbilityAction()
...
function UpdateCreateCharacterBtnState()
  ...
  local convertText = X2Util:CheckAvailableName(text, step3.content.nameWnd.characterName:IsComposingText())
  ...
  if characterCreate.state == "Step3" then
    if X2World:IsPreSelectCharacterPeriod() then
      if X2World:GetCurrentTotalCharactersCount() >= X2World:GetCurrentTotalCharactersLimit() and not X2LoginCharacter:IsRecustomizing() then
    ...
    elseif allValid and not characterCreate.createRequested then
      characterCreate.bgWindow.stepCompleteButton:Enable(true)
      step3.content.rightPanel.startButton:Enable(true)
    else
      characterCreate.bgWindow.stepCompleteButton:Enable(false)
      step3.content.rightPanel.startButton:Enable(false)
    end
  end
end
...
local function GetPossibleRaceIndex()
...
local function CheckWorldEntranceQueue()
...
local eventHandler = {
  SHOW_CHARACTER_CREATE_WINDOW = function(show)
    ...
    if show then
      isBusy = false
      narrationSoundIndex = PlayUISound(narrationSoundName)
    else
      StopSound(narrationSoundIndex)
  ...
  SHOW_CHARACTER_ABILITY_WINDOW = function(show)
    ...
    if show then
      isBusy = false
      abilityNarrationSoundIndex = PlayUISound(abilityNarrationSoundName)
    else
      StopSound(abilityNarrationSoundIndex)
