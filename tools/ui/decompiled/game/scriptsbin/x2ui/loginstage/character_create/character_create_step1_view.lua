...
function ChangeRaceName(index)
  ...
  local coords_table = {
    {
      GetCoordInfo(path, "nuian"):GetCoords()
    },
    {
      0,
      0,
    ...
    {
      GetCoordInfo(path, "dwarf"):GetCoords()
    },
    {
      GetCoordInfo(path, "elf"):GetCoords()
    },
    {
      GetCoordInfo(path, "harani"):GetCoords()
    },
    {
      GetCoordInfo(path, "firran"):GetCoords()
    },
    {
      0,
      0,
    ...
    {
      GetCoordInfo(path, "warborn"):GetCoords()
    }
  }
...
function ShowCongestionTip(target, congestion)
...
function UpdateSelectableRace()
  ...
  if isPreSelectPeriod and isRecustomizing then
  ...
  for i = 1, #RACE_TYPE do
    ...
    if isRecustomizing then
      if recustomingRace ~= nil and i ~= recustomingRace then
        enable = false
      end
    elseif IsUnableRaceToCreate(RACE_TYPE[i]) then
      enable = false
    end
    if characterCreate.genderIndex == GENDER_FEMALE and RACE_TYPE[i] == RACE_WARBORN then
      enable = false
    end
    btn:Enable(enable)
    ...
    function btn:OnEnter()
      ...
      if IsUnableRaceToCreate(RACE_TYPE[i]) or isPreSelectPeriod then
        ShowCongestionTip(btn, congestion[RACE_TYPE[i]])
      end
    end
...
desc:SetExtent(desc:GetLongestLineWidth() + 5, desc:GetTextHeight())
desc:AddAnchor("BOTTOMLEFT", continentDescFrame, 0, 0)
desc.style:SetShadow(true)
desc.style:SetAlign(ALIGN_LEFT)
local width = math.max(desc:GetWidth(), nuiaImg:GetWidth(), hariharaImg:GetWidth())
local height = nuiaImg:GetHeight() + desc:GetHeight() + 20
continentDescFrame:SetExtent(width, height)
