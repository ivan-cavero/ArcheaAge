...
function IsValidLoginDataLength()
  ...
  if 4 <= idLen and 6 <= passwdLen then
    return true
  elseif 4 <= idLen and 1 <= passwdLen then
    HasPwdFocus = true
  end
  return false
end
...
function Trylogin()
  if IsValidLoginDataLength() and isReadyToLogin then
    ...
    passwdEdit:SetText("")
    PlayUISound("login_stage_try_login", true)
...
local function HideStaffWindows()
  login.state = "STAFF_FADE_END"
  StopMusic()
  PlayMusic("login_stage_music_before_login")
...
local eventHandler = {
  ...
  OPEN_OTP = function(currentTry, maxTry, onTime)
    ...
    if onTime == true and 1 < currentTry then
